...
class OptimisticTransactionManager {
  ...
  private notify(txId: string, status: "pending" | "confirmed" | "failed", tx: PendingTransaction) {
  ...
  private async pollTransaction(txId: string, attempt = 0) {
    ...
    try {
      ...
      if (attempt < 30) {
        const interval = setTimeout(() => this.pollTransaction(txId, attempt + 1), 1500)
...
export function useOptimisticTransactions(poolAddress: string) {
  ...
  const setOptimisticValues = useCallback((values: Omit<OptimisticUpdate, "pendingTx">) => {
    if (!txIdRef.current) return
  ...
  const markFailed = useCallback((error: string) => {
    ...
    setOptimisticState((prev) => ({
      pendingTx: prev.pendingTx ? { ...prev.pendingTx, status: "failed", error } : undefined,
    }))
  }, [])
